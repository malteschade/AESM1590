classdef InteractiveColorbar < hgsetget
    ...
    properties (Access=private)
        ...
        ResizeListener
        %Listener for resizes of figure, axes & colorbar needs resizing then
        ResizeFigureListener
        %Listen for deletion of axes, delete colorbar if so
        DeleteListener
        %Colormaps for context menu (right click on colorbar)
        ColorMaps={'Gray','Inverted Gray','Seismic','Colormap Editor'}
        %Handle for the context menu
        ContextMenu
        %Handle for alternative WindowButtonMotionFcn/WindowButtonUpFcn
        AltWBMF
        AltWBUF
        %Normalized Original Figure Size
        OrigFigPos=[0,0,0,0];
    end
    ...
    properties(Access=public)
        ...
        %Set if line motion should be symmetric around center
        Sym=false;
    end 
    ...
    methods 
        ...
        function obj=InteractiveColorbar(AxesHandle)
            ...
            obj.OrigFigPos=get(gcf,'Position'); %Save original figure size for resizing
...
%            obj.ResizeFigureListener=addlistener(obj.FigureHandle,'Position','PostSet',@obj.ResizeFcn);
            obj.ResizeFigureListener=addlistener(obj.FigureHandle,'SizeChanged',@obj.ResizeFcn);
        ...
        function delete(obj)
            ...
            delete(obj.ResizeFigureListener);
        ...
        function set.ColorMap(obj,ColorMapStr)
            switch lower(ColorMapStr)
                case 'seismic'
                    colormap(obj.seismic);
                case 'inverted gray'
                    CMap=1-(0:1/63:1)';
        ...
        function set.AltWBMF(obj,AltWBMF)
            obj.AltWBMF=AltWBMF;
        end
        function AltWBMF=get.AltWBMF(obj)
            AltWBMF=obj.AltWBMF;
        end
        function set.AltWBUF(obj,AltWBUF)
            obj.AltWBUF=AltWBUF;
        end
        function AltWBUF=get.AltWBUF(obj)
            AltWBUF=obj.AltWBUF;
        end
    end
    methods (Access=public)
        ...
        function ResizeFcn(obj,~,~)
            ...
            FPosOrig=obj.OrigFigPos;
            obj.OrigFigPos=FPos;
            AxPos=get(obj.AxesHandle,'Position');
            AxPos([1,3])=AxPos([1,3])./FPosOrig(3).*FPos(3);
            AxPos([2,4])=AxPos([2,4])./FPosOrig(4).*FPos(4);
            set(obj.AxesHandle,'Position',AxPos);
        end
    end
    methods (Access=private)
        ...
        function CreateContextMenu(obj)
            ...
            uimenu(obj.ContextMenu,'Label','Set Limits','CallBack',@obj.ContextMenuCallback);
            uimenu(obj.ContextMenu,'Label','Reset Limits','CallBack',@obj.ContextMenuCallback);
            SymHandle=uimenu(obj.ContextMenu,'Label','Symetric Clipping','CallBack',@obj.ContextMenuCallback);
            ...
            function SetCallBackMenu(~,~,~)
                ...
                if obj.Sym;checkstatus='on';else checkstatus='off';end
                set(SymHandle,'Checked',checkstatus);
            end
        end
        ...
        function LineClick(obj,source,~)
            obj.AltWBMF=get(gcf,'WindowButtonMotionFcn');
            obj.AltWBUF=get(gcf,'WindowButtonUpFcn');
            set(gcf,'WindowButtonMotionFcn',{@obj.MouseMove,source});
            set(gcf,'WindowButtonUpFcn',@obj.ButtonRelease);
        end
        function ButtonRelease(obj,~,~)
            set(gcf,'WindowButtonMotionFcn',obj.AltWBMF)
            set(gcf,'WindowButtonUpFcn',obj.AltWBUF)
        end
        function MouseMove(obj,~,~,source)
            ...
            switch index
                case {1}
                    ...
                    if obj.Sym
                       Range=obj.CRange;
                       obj.CLim(2)=Range(2)+Range(1)-newLim;
                    end
                ...
                case {2}
                    ...
                    if obj.Sym
                       Range=obj.CRange;
                       obj.CLim(1)=Range(1)+Range(2)-newLim;
                    end
                    notify(obj,'UpdateByUser')
            end
        end
        function ContextMenuCallback(obj,source,~)
            ...
            switch lower(ColorMapStr)
                ...
                case 'set limits'
                    % Allow for colorbar range to be set to current CLim
                    % This allows for better clipping when CLim range
                    % is very small.
                    SetCRange(obj,obj.CLim,true);
                case 'reset limits'
                    % Reset colorbar range to original
                    SetCRange(obj,obj.OriginalCRange,true);
                case 'symetric clipping'
                    % Toggles if symmetric clipping is in use
                    if obj.Sym;
                        obj.Sym=false;
                    else
                        obj.Sym=true;
        ...
        function map=seismic(~,m)
            % seismic colormap from:
            % https://csdms.colorado.edu/svn/sedflux/sedflux-mfiles/sedflux/seismic.m (13.10.2014)
            if nargin<2;
               m=size(get(gcf,'colormap'),1);
            end
            r=interp1([1/m,0.5,1]*m,[1,1,0],1:m);
            g=interp1([1/m,0.5,1]*m,[0,1,0],1:m);
            b=interp1([1/m,0.5,1]*m,[0,1,1],1:m);
            a=find(r==1&g==1&b==1);
            r(a-1:a+1)=1;g(a-1:a+1)=1;b(a-1:a+1)=1;
            map=[r',g',b'];
        end
    end
end